const input = `iyr:2015 cid:189 ecl:oth byr:1947 hcl:#6c4ab1 eyr:2026
hgt:174cm
pid:526744288
...
for (i=0; i<inputs.length; i++){
    for (j=0; j<inputs[i].length; j++){
        if (inputs[i].charAt[j] == 'slash' &&  inputs[i].charAt[j+1] == "n") {
            //pop the shit out the string.
        }
    }
}

//Constructor for making passport objects:
function passport(byr,iyr,eyr,hgt,hcl,ecl,pid,cid){
    this.birthYear = byr,
    this.issueYear = iyr,
